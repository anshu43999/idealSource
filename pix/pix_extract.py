...
def normalize_pre_proxy_url(proxy: str) -> str:
    ...
    previous_scheme = os.environ.get("PIX_PROXY_DEFAULT_SCHEME")
    os.environ["PIX_PROXY_DEFAULT_SCHEME"] = "socks5h"
    try:
        return normalize_proxy_url(proxy)
    finally:
        if previous_scheme is None:
            os.environ.pop("PIX_PROXY_DEFAULT_SCHEME", None)
        else:
            os.environ["PIX_PROXY_DEFAULT_SCHEME"] = previous_scheme
