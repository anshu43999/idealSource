...
def checkout_proxy_file() -> Path:
    raw = os.environ.get("UPI_CHECKOUT_PROXY_FILE", "").strip()
    return Path(raw).expanduser() if raw else SCRIPT_DIR / "in_proxy_seeds.txt"


def promotion_proxy_file() -> Path:
    raw = os.environ.get("UPI_PROMOTION_PROXY_FILE", "").strip()
    return Path(raw).expanduser() if raw else SCRIPT_DIR / "vn_proxy_seeds.txt"


def provider_proxy_file() -> Path:
    raw = os.environ.get("UPI_PROVIDER_PROXY_FILE", "").strip()
