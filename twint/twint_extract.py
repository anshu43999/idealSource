...
def normalize_pre_proxy_url(proxy: str) -> str:
    ...
    previous_scheme = os.environ.get("TWINT_PROXY_DEFAULT_SCHEME")
    os.environ["TWINT_PROXY_DEFAULT_SCHEME"] = "socks5h"
    try:
        return normalize_proxy_url(proxy)
    finally:
        if previous_scheme is None:
            os.environ.pop("TWINT_PROXY_DEFAULT_SCHEME", None)
        else:
            os.environ["TWINT_PROXY_DEFAULT_SCHEME"] = previous_scheme
...
def checkout_proxy_file() -> Path:
    raw = os.environ.get("TWINT_CHECKOUT_PROXY_FILE", "").strip()
    return Path(raw).expanduser() if raw else SCRIPT_DIR / "ch_proxy_seeds.txt"


def promotion_proxy_file() -> Path:
    raw = os.environ.get("TWINT_PROMOTION_PROXY_FILE", "").strip()
    return Path(raw).expanduser() if raw else SCRIPT_DIR / "vn_proxy_seeds.txt"


def provider_proxy_file() -> Path:
    raw = os.environ.get("TWINT_PROVIDER_PROXY_FILE", "").strip()
